...
with tf.name_scope(name="Hidden_layer"):
    ...
    hidden_layer_in = tf.matmul(X, w1) + b1
    hidden_layer_output = tf.tanh(hidden_layer_in)

with tf.name_scope("output_layer"):
    w2 = tf.Variable(tf.random_normal(shape=(n_hidden_layer, 10), mean=0, stddev=std, seed=100), name="W")
    b2 = tf.Variable(tf.zeros([10]), name="B")  # 10, is the number of classes since we are classifying the data into 10
    #  different classes as is mentioned in the pdf file.

    output_layer_in = tf.matmul(hidden_layer_output, w2) + b2
...
confusion_matrix = tf.confusion_matrix(tf.argmax(Y, 1), tf.argmax(output, 1), num_classes=10)
...
with tf.Session() as sess:
    ...
    summary_writer = tf.summary.FileWriter('500_neuron_tanh_loss_event')
    # ########################### Training ###########################
    ...
    while counter <= len(x_validation):
        ...
        summary_writer.add_summary((sess.run(validation_loss_summary, feed_dict={X: x_batch, Y: y_batch})), counter)
    ...
    cm = sess.run(confusion_matrix, feed_dict={X: x_batch, Y: y_batch})
    print(cm)
